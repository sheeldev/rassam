...
if (!empty($_SESSION['sheeldata']['user']['userid']) and $_SESSION['sheeldata']['user']['userid'] > 0 and $_SESSION['sheeldata']['user']['isadmin'] == '1') {
    ...
    $migrate_count = 0;
    $languages_role = $languages_plan = $languages_permission_group = $subscription_roles = $subscription_rows = $subscription_group_rows = $access_permission_groups = $r = array();
    ...
    if (isset($sheel->GPC['subcmd']) AND $sheel->GPC['subcmd'] == 'delete')
    {
        ...
        else if (isset($sheel->GPC['type']) AND $sheel->GPC['type'] == 'permission')
        {
            $sheel->db->query("
                ...
                WHERE subscriptiongroupid = '" . intval($sheel->GPC['subscriptiongroupid']) . "'
            ");
            $sheel->db->query("
    ...
    else if (isset($sheel->GPC['subcmd']) AND $sheel->GPC['subcmd'] == 'update')
    {
        if (isset($sheel->GPC['type']) AND $sheel->GPC['type'] == 'plan')
        {
            ...
            else
            {
                ...
                if ($sheel->db->num_rows($sql) > 0)
                {
                    ...
                    $sheel->GPC['form']['subscriptiongroupid'] = (isset($sheel->GPC['form']['subscriptiongroupid']) ? intval($sheel->GPC['form']['subscriptiongroupid']) : '');
        ...
        {
            $sheel->template->meta['areatitle'] = 'Admin CP | Marketplace - Membership Plans - Update Permission';
            $sheel->template->meta['pagetitle'] = SITE_NAME . ' - Admin CP | Marketplace - Membership Plans - Update Permission';
            $groupname = $sheel->db->fetch_field(DB_PREFIX . "subscription_group", "subscriptiongroupid = '" . intval($sheel->GPC['subscriptiongroupid']) . "'", "title_" . $_SESSION['sheeldata']['user']['slng']);
            $currentarea = '<span class="breadcrumb"><a href="' . HTTPS_SERVER_ADMIN . 'settings/memberships/">{_subscription_plans}</a> / </span> ' . $groupname;
            if (isset($sheel->GPC['do']) AND $sheel->GPC['do'] == 'save')
            {
                $field = '';
                $sql_lang = $sheel->db->query("SELECT languagecode, title FROM " . DB_PREFIX . "language");
                while ($res_lang = $sheel->db->fetch_array($sql_lang, DB_ASSOC))
                {
                    ...
                    if (!isset($sheel->GPC['form']['description_' . $languagecode]) OR empty($sheel->GPC['form']['description_' . $languagecode]) OR !isset($sheel->GPC['form']['title_' . $languagecode]) OR empty($sheel->GPC['form']['title_' . $languagecode]) )
                    {
                        $sheel->admincp->print_action_failed('{_please_fill_all_fields}', HTTPS_SERVER_ADMIN . 'settings/memberships/');
                        exit();
                    }
                    $field .= "title_" . $languagecode . " = '" . $sheel->db->escape_string($sheel->GPC['form']['title_' . $languagecode]) . "', description_" . $languagecode . " = '" . $sheel->db->escape_string($sheel->GPC['form']['description_' . $languagecode]) . "',";
                }
            ...
            {
                $form['subscriptiongroupid'] = $sheel->GPC['subscriptiongroupid'];
                $sql = $sheel->db->query("
                    SELECT *
        ...
        {
            $sheel->GPC['subscriptiongroupid'] = $form['subscriptiongroupid'] = (isset($sheel->GPC['subscriptiongroupid']) ? $sheel->GPC['subscriptiongroupid'] : 0);
            $sheel->template->meta['areatitle'] = 'Admin CP | Marketplace - Membership Plans - Update Permissions';
            $sheel->template->meta['pagetitle'] = SITE_NAME . ' - Admin CP | Marketplace - Membership Plans - Update Permissions';
            $groupname = $sheel->db->fetch_field(DB_PREFIX . "subscription_group", "subscriptiongroupid = '" . intval($sheel->GPC['subscriptiongroupid']) . "'", "title_" . $_SESSION['sheeldata']['user']['slng']);
            $currentarea = '<span class="breadcrumb"><a href="' . HTTPS_SERVER_ADMIN . 'settings/memberships/">{_subscription_plans}</a> / <a href="' . HTTPS_SERVER_ADMIN . 'settings/memberships/update/permission/' . intval($sheel->GPC['subscriptiongroupid']) . '/">' . $groupname  . '</a> / </span> {_permissions}';
            $acpaccess_json = "''";
            if (isset($sheel->GPC['do']) AND $sheel->GPC['do'] == 'save')
            {
                $sql = $sheel->db->query("
                ...
                { // update permissions
                        ...
                        if (isset($k) AND !is_array($v))
                        {
                            $sheel->db->query("
                                ...
                                WHERE accessname = '" . $sheel->db->escape_string($k) . "'
                                    AND subscriptiongroupid = '" . intval($sheel->GPC['subscriptiongroupid']) . "'
                                LIMIT 1
                            ");
                        }
                    }
                    // acp permissions
                    $acpaccess = '';
                    if (isset($sheel->GPC['acpaccess']) AND is_array($sheel->GPC['acpaccess']))
                    {
                        $acpaccess = json_encode($sheel->GPC['acpaccess']);
                    }
                    else
                    {
                        $acpaccess = json_encode(array());
                    }
                    $sheel->db->query("
                        UPDATE " . DB_PREFIX . "subscription_permissions
                        SET value = '" . $sheel->db->escape_string($acpaccess) . "'
                        WHERE accessname = 'acpaccess'
                            AND subscriptiongroupid = '" . intval($sheel->GPC['subscriptiongroupid']) . "'
                        LIMIT 1
                    ");
                }
                ...
                { // create new permissions
                    $sql = $sheel->db->query("
                        SELECT id, subscriptiongroupid, accessname, accessgroup, accesstype, accessmode, value, original, iscustom, visible
                        FROM " . DB_PREFIX . "subscription_permissions
                    ...
                    if ($sheel->db->num_rows($sql) > 0)
                    {
                        while ($res = $sheel->db->fetch_array($sql, DB_ASSOC))
                        {
                            $sheel->db->query("
                                ...
                                VALUES(
                                NULL,
                                '" . intval($sheel->GPC['subscriptiongroupid']) . "',
                        ...
                        foreach ($sheel->GPC['form'] AS $k => $v)
                        { // update permissions with pre-configured settings the admin may have enabled/disabled
    ...
    else if (isset($sheel->GPC['subcmd']) AND $sheel->GPC['subcmd'] == 'add')
    {
        if (isset($sheel->GPC['type']) AND $sheel->GPC['type'] == 'plan')
        {
            ...
            else
            {
                ...
                $form['cost'] = '0.00';
                for ($i = 1;$i < 31;$i++){$arr[$i] = $i;} // <-- base this on selected payment gateway rules
            ...
            if (isset($sheel->GPC['do']) AND $sheel->GPC['do'] == 'save')
            {
                ...
                else
                {
                    $sheel->db->query("
                        ...
                        (subscriptiongroupid, " . $field1 . "canremove, type)
                        VALUES(
                        NULL,
                        " . $field2 . "
                        '1',
        ...
        if (is_array($_FILES) AND isset($_FILES['upload']['tmp_name']))
		{
			...
			{
				if ($fileinfo = getimagesize($_FILES['upload']['tmp_name']))
				{ // only accept image
					if (isset($fileinfo[0]) AND isset($fileinfo[1]) AND $fileinfo[0] > 0 AND $fileinfo[1] > 0)
					{
						...
						if ($fileinfo[2] != 1 AND $fileinfo[2] != 2 AND $fileinfo[2] != 3)
						{ // error not .jpg, .gif or .png
							$array = array(
								'error' => 1,
    ...
    if ($sheel->db->num_rows($sql) > 0)
    {
        while ($row = $sheel->db->fetch_array($sql, DB_ASSOC))
        {
            ...
            {
                $row['access'] = ' <a href="' . HTTPS_SERVER_ADMIN . 'settings/settings/update/permission/' . $row['subscriptionid'] . '/">{_set_up}</a>';
            }
            else {
                $row['access']= ' <a href="' . HTTPS_SERVER_ADMIN . 'settings/settings/update/permission/' . $row['subscriptionid'] . '/">{_update}</a>';
            }
    ...
    $sheel->template->parse_loop('main', array(
        ...
        'subscription_group_rows' => $subscription_group_rows,
