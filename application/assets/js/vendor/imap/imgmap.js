...
imgmap.prototype.setup = function(config) {
	...
	if (this.isMSIE &&
		...
		this.loadScript(this.config.baseroot + 'excanvas' + ((iL['MINIFY'] == '1') ? '.min.js' : '.js'));
	}
	...
	if (typeof imgmapStrings == 'undefined') {
		...
		this.loadScript(this.config.baseroot + 'lang_' + this.config.lang + ((iL['MINIFY'] == '1') ? '.min.js' : '.js'));
	}
...
imgmap.prototype.getMapHTML = function(flags) {
	var html = '<map id="hero_{id}" name="hero_{id}">' + this.getMapInnerHTML(flags) + '</map>';
