...
function defaultagree(formobj) { // attach to onsubmit="return defaultagree()"
...
function toggler(target) {
	if (jQuery('#' + target).length) {
		...
		else { // backward compatibility for style="display:none"..
			obj = fetch_js_object(target);
			obj.style.display = (obj.style.display == 'none') ? 'inline' : 'none';
		}
	}
...
function validate_payment_profile(formobj, escrow, directpayment, gateway, ia) {
	...
	if (directpayment && fetch_js_object('enableescrow4').checked == true) { // gateway == 'paypal_pro' && ia == 1
