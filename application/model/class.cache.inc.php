...
class cache
{
	...
	function __construct($sheel)
	{
		...
		switch ($engine) {
			...
			case 'apc': { // should never be useed on shared server
				$this->name = (extension_loaded('apc') and ini_get('apc.enabled')) ? 'sheel_apc' : 'sheel_filecache';
				break;
			}
			case 'memcached': {
				$this->name = 'sheel_memcached';
				break;
			}
			case 'none': {
